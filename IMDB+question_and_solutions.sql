...
WITH actor_summary AS (
    ...
    WHERE M.country = 'india'  -- Considering only movies released in India
...
SELECT ASum.actress_name,
       ASum.total_votes,
       ASum.movie_count,
       WR.actress_avg_rating,
       RANK() OVER (ORDER BY WR.actress_avg_rating DESC) AS actress_rank
FROM actress_summary AS ASum
JOIN weighted_avg_ratings AS WR ON ASum.actress_name = WR.actress_name
LIMIT 5;
...
movie_summary AS (
    ...
    GROUP BY g.genre, m.year, movie_name, worlwide_gross_income
)
SELECT *
FROM movie_summary
WHERE movie_rank <= 5
